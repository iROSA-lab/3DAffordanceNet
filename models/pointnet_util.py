...
def index_points(points, idx):
    ...
    if idx.dim() == 2:
        ...
        new_points = gather_operation(points, idx).transpose(1, 2).contiguous()
    elif idx.dim() == 3:
        points = points.transpose(1, 2).contiguous()
        new_points = grouping_operation(
            points, idx).permute(0, 2, 3, 1).contiguous()
    # idx = idx.long()
...
class PointNetSetAbstractionMsg(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        B, N, C = xyz.shape
        S = self.npoint
        new_xyz = index_points(xyz, furthest_point_sample(xyz, S))
        new_points_list = []
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if S == 1:
            ...
            #     points2, idx, weight).permute(0, 2, 1)
